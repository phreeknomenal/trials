  ...
  erb_template <<-ERB
    <header class="border-b border-zinc-200 dark:border-zinc-700"
      ...
      <div class="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4 py-4">
        ...
        <button type="button"
                class="lg:hidden inline-flex items-center justify-center w-10 h-10 rounded-md text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-lavender-600"
  ...
  def render_account_control
    if user_signed_in?
      link_to helpers.profile_path(current_profile),
        class: "block w-10 h-10 rounded-full overflow-hidden focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-lavender-600",
      ...
      link_to "Login", helpers.new_user_session_path,
        class: "bg-lavender-600 text-white px-4 py-2 font-medium rounded-md hover:bg-lavender-700 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-lavender-600"
    end
  end
end
