class Page::Cards::IconCardComponent < ApplicationComponent
  erb_template <<-ERB
    <% if style == :default %>
      <div class="w-full h-full flex gap-x-4 border border-line-2 dark:border-line-on-dark rounded-card p-6 space-y-3">
    ...
    <% elsif style == :top %>
      <div class="w-full h-full flex flex-col gap-x-4 border border-line-2 dark:border-line-on-dark rounded-card p-6 space-y-3">
        ...
        <div class="mt-auto">
          <%= link_to path, class: "flex items-center gap-1 font-semibold text-lg text-sky-600 hover:text-sky-700 transition group" do %>
